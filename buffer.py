...
def main(args):
    ...
    for it in range(0, args.num_experts):
        ...
        # teacher_net = torchvision.models
        if args.model == 'pretrained':
            teacher_net = torchvision.models.resnet18(pretrained=True)
            fc_features = teacher_net.fc.in_features
            teacher_net.fc = torch.nn.Linear(fc_features, 100)
            optim_list = []
            for name, param in teacher_net.named_parameters():
                if name.find("fc")!=0:
                    param.requires_grad = False
                else:
                    optim_list.append(param)
            teacher_net.train()
            lr = args.lr_teacher
            teacher_optim = torch.optim.SGD(optim_list , lr=lr, momentum=args.mom, weight_decay=args.l2)
        ...
        timestamps = []
        if args.model == 'pretrained':
            timestamps.append([p.detach().cpu() for p in optim_list])
        else:
