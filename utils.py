...
"""
有修改
    在get_dataset函数中添加了几个新数据集‘SVHN’、'MSTAR_32'、'MSTAR'、'EuroSAT_32'
    增加了SoftLabelDataset类
    在epoch函数中增加了使用软标签的情况
    在evaluate_synset增加了使用软标签的情况
    增加了one_hot_CrossEntropy类
"""
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
    ...
    elif dataset.startswith('MSTAR_32'):
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        transform = transforms.Compose([
            ...
            transforms.ToTensor()])
        mean = [0.2331, 0.2331, 0.2331]
        std = [0.1606, 0.1606, 0.1606]
        # 创建ImageFolder数据集
        # print("当前工作目录:", os.getcwd())
        dst_train = torchvision.datasets.ImageFolder(root='data/MSTAR/train', transform=transform)
        dst_test = torchvision.datasets.ImageFolder(root='data/MSTAR/val', transform=transform)
        class_names = None
        class_map = {x: x for x in range(num_classes)}
        # train_loader = DataLoader(dataset=train_dataset, batch_size=32, shuffle=True)
        # val_loader = DataLoader(dataset=val_dataset, batch_size=32, shuffle=False)
    elif dataset.startswith('MSTAR'):
        channel = 3
        im_size = (64, 64)
        num_classes = 10
        transform = transforms.Compose([
        ...
        class_names = None
        class_map = {x: x for x in range(num_classes)}
    elif dataset.startswith('EuroSAT_32'):
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        transform = transforms.Compose([
            transforms.Resize((32,32)),
            transforms.ToTensor()])
        mean = [0.3444, 0.3804, 0.4077]
        std = [0.0931, 0.0647, 0.0542]
        # 创建ImageFolder数据集
        # print("当前工作目录:", os.getcwd())
        dst_train = torchvision.datasets.ImageFolder(root='data/EuroSAT/train', transform=transform)
        dst_test = torchvision.datasets.ImageFolder(root='data/EuroSAT/val', transform=transform)
        class_names = None
        class_map = {x: x for x in range(num_classes)}
    else:
    ...
    # testloader = torch.utils.data.DataLoader(dst_test, batch_size=128, shuffle=False)
...
class SoftLabelDataset(Dataset):
    def __init__(self, data, labels, soft_labels):
        self.data = data
        self.labels = labels
    ...
    def __getitem__(self, idx):
        ...
        return sample, label, soft_label

def get_time():
    return str(time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime()))


def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False, temperature=1):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if len(datum)==3:
            soft_lab = datum[2].float().to(args.device)
        ...
        if len(datum)==3:
            ...
            q = torch.softmax(soft_lab/temperature, dim=-1)
            kl_loss = torch.sum(p * (log_p - torch.log(q + 1e-10)), dim=-1).mean()
            lab = torch.argmax(soft_lab.detach(), axis=-1)
            loss = 0.2*criterion(output, lab) + kl_loss
            # loss = kl_loss
        else:
...
def evaluate_synset(it_eval, net, images_train, labels_train, labels_soft, testloader, args, return_loss=False, texture=False, temperature=1):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    if args.soft_label:
        labels_soft = labels_soft.to(args.device)
    ...
    if args.soft_label:
        dst_train = SoftLabelDataset(images_train, labels_train, labels_soft)
    else:
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, args, aug=True, texture=texture, temperature=temperature)
